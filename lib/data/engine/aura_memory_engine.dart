...
class AuraMemoryEngine {
  ...
  String getBestTag(AuraState state, TimeContext time, WeatherContext weather, String countryCode) {
    ...
    List<String> userTags = _prefs.getStringList(key) ?? _generateSmartTags(state, time, weather, countryCode);
    return userTags.first;
  }

  Future<void> penalizeTag(AuraState state, TimeContext time, WeatherContext weather, String countryCode, String dislikedTag) async {
    String key = _getKey(state, time, weather);
    List<String> tags = _prefs.getStringList(key) ?? _generateSmartTags(state, time, weather, countryCode);
